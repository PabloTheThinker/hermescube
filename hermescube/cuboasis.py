"""Cuboasis — pocket-dimension memory infrastructure inside HermesCube.

The cube is one warehouse file, but inside it lives a custom oasis:
  SPACE       — vaults + chambers (organization without a second store)
  WAVE        — Cubewave neural field (brainwave-like associations)
  CONNECTIONS — unified neighbors across SPO / colony / engram / Cubewave / HAR
  PROGRESS    — append-only ledger proving the loop improved usefulness

This is not a generic "nexus". Cuboasis is the HermesCube-native spine that
makes space, wave, connections, and progress one navigable system for agents.
"""
...
# Backward-compat alias (Nexus → Cuboasis rename)
nexus_state_path = cuboasis_state_path
...
def space_map(
    ...
    *,
    ...
    if isinstance(topics_raw, dict):
        topic_list = sorted(
            topics_raw.keys(), key=lambda k: -len(topics_raw.get(k) or [])
        )[:12]
...
def filter_by_chamber(
    scored: list[tuple[Any, float]],
    chamber: str,
    *,
    soft: bool = True,
) -> list[tuple[Any, float]]:
    """Keep (or prefer) entries in a chamber. soft=True boosts rather than hard-cuts."""
    from hermescube.living import _chamber_of

    want = (chamber or "").strip().lower()
    if not want or not scored:
        return scored
    if soft:
        out: list[tuple[Any, float]] = []
        for e, s in scored:
            if _chamber_of(e) == want:
                out.append((e, float(s) * 1.18))
            else:
                out.append((e, float(s) * 0.92))
        out.sort(key=lambda x: (-x[1], getattr(x[0], "description", "") or "", getattr(x[0], "id", "") or ""))
        return out
    kept = [(e, s) for e, s in scored if _chamber_of(e) == want]
...
def connect_entity(
    ...
    *,
    ...
    """Unified neighbor view: SPO + colony + engram + Cubewave + HAR related."""
    ...
    try:
        if cubewave is not None and cube is not None:
            by_id = {str(e.id): e for e in list(cube.read_l1() or [])}
            for hid in cubewave.hub_ids(limit=limit):
                e = by_id.get(str(hid))
                if e is None:
                    continue
                desc = (e.description or "")[:160]
        ...
        logger.debug("connect cubewave skip: %s", e)
    ...
    return {
        ...
        "cubewave_hubs": waves[:limit],
        "related": related[:limit],
        "counts": {
            "spo": len(spo),
            "colony": len(trails),
            "engram_hubs": len(hubs),
            "cubewave_hubs": len(waves),
            "related": len(related),
        },
    }
...
def bridge_claim_to_relation(
    ...
    *,
    ...
    try:
        ...
        d = {}
    # Prefer first-class SPO fields; fall back to meta / text parse
    subj = str(
        d.get("subject")
        or (d.get("meta") or {}).get("subject")
        or d.get("entity")
        or ""
    ).strip()
    pred = str(
        d.get("predicate")
        or (d.get("meta") or {}).get("predicate")
        or d.get("relation")
        or "claims"
    ).strip()
    obj = str(
        d.get("object")
        or (d.get("meta") or {}).get("object")
        or d.get("value")
        or d.get("statement")
        or ""
    ).strip()
    if not obj and d.get("text"):
        obj = str(d["text"])[:200]
    if (not subj or not obj or pred == "claims") and d.get("text"):
        try:
            from hermescube.claims import infer_spo_from_text

            inferred = infer_spo_from_text(str(d.get("text") or ""))
            if inferred:
                subj = subj or inferred[0]
                pred = pred if pred and pred != "claims" else inferred[1]
                obj = obj if obj and obj != str(d.get("text") or "")[:200] else inferred[2]
        except Exception:
            pass
    if not subj or not obj:
        ...
        obj = text[:200]
    mid = str(d.get("memory_id") or d.get("entry_id") or d.get("id") or d.get("claim_id") or "") or None
...
def progress_usefulness(
    ...
    *,
    ...
    """Rollup helpful/unhelpful from the progress ledger (for capability scoring)."""
    p = progress_path(
        hermes_home,
        agent_identity=agent_identity,
        agent_workspace=agent_workspace,
    ...
    rate = round(helpful / total, 3) if total > 0 else None
    return {
        "events": events,
        "helpful": helpful,
        "unhelpful": unhelpful,
...
def progress_status(
    ...
    *,
    ...
    useful = progress_usefulness(hermes_home, **pkw)
    ...
    return {
        ...
        "kinds": dict(kinds),
        "recent": recent,
        "usefulness": useful.get("usefulness"),
        "helpful": useful.get("helpful", 0),
        "unhelpful": useful.get("unhelpful", 0),
        "growth": {
...
def cuboasis_status(
    ...
    *,
    ...
    try:
        if cubewave is not None and hasattr(cubewave, "stats"):
            wave_stats = cubewave.stats()
    except Exception:
        wave_stats = {}
    ...
    status = {
        ...
        "space": space,
        "wave": wave_stats,
        "active_chamber": active_chamber or "",
        "connections": {
            "relations": rel_stats.get("relations", 0),
            "relations_open": rel_stats.get("open", 0),
            "colony_edges": colony_n,
            "engram_nodes": engram_n,
            "cubewave_readouts": wave_stats.get("readouts", 0),
            "cubewave_edges": wave_stats.get("edges", 0),
        },
        "progress": {
    ...
    if hermes_home and not light:
        try:
            ...
            # Migrate legacy nexus_state.json once if present and new missing was written
            legacy = sp.parent / "nexus_state.json"
            if legacy.is_file() and legacy.resolve() != sp.resolve():
                # keep both; cuboasis is canonical
                pass
        except OSError as e:
            logger.debug("cuboasis state write skip: %s", e)

    return status


# Backward-compat
nexus_status = cuboasis_status
